...
Update documentation in the same change when commands, environment variables, APIs, route contracts, lifecycle states, user workflows, safety boundaries, or operating procedures change. Internal changes with no reader-visible documentation impact do not require a docs edit. Run both `npx mintlify@latest validate` and `npx mintlify@latest broken-links` before release.
